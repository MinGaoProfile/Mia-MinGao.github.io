...
function BibtexParser() {
    ...
    this.entry_body = function (directive) {
        ...
        } else if (directive == "@PATENT") {
            this.entries[this.currentEntry]["BIBTEXTYPE"] = "patent";
        } else if (directive == "@AWARD") {
...
function BibtexDisplay() {
    ...
    this.createTemplateWorks = function (entry, output, tmplateTag, divclass) {
        // Check if bibtex keys are limiting output (bibtexkeys="key1|key2|...|keyN")
        if (output[0].attributes.length > 1) {
            for (var i = 0, attrs = output[0].attributes, n = attrs.length; i < n; ++i) {
                var name = attrs[i].nodeName.toUpperCase();
        ...
        // find template
        var tpl = $("." + tmplateTag).clone().removeClass(tmplateTag);
    ...
    this.createArrayPaper = function (entries) {
        var entriesArray = [];
        for (var entryKey in entries) {
    ...
    this.createArrayPatent = function (entries) {
        var entriesArray = [];

        for (var entryKey in entries) {
            if (entries[entryKey].BIBTEXTYPEKEY == 'PATENT')
                entriesArray.push(entries[entryKey]);
        }

        return entriesArray;
    }
    this.createArrayAward = function (entries) {
        var entriesArray = [];
        for (var entryKey in entries) {
            if (entries[entryKey].BIBTEXTYPEKEY == 'AWARD')
                entriesArray.push(entries[entryKey]);
        }
        return entriesArray;
    }
    ...
    this.createStructure = function (structure, output, entries, level) {
        ...
        if (groupChild.length) {
            ...
            sortedArray = this.sortArray(entries, sortName, rule, type);
            if (level == 0) {
    ...
    this.displayBibtex = function (input, output) {
        ...
        if (structure.length) {
            ...
            var entriesArray = this.createArrayPaper(entries);
            sortedArray = this.sortArray(entriesArray, 'DATE', 'DESC', 'date');
            this.createStructure(structure, output, sortedArray);

        } else {
            // iterate over bibTeX entries
            for (var entryKey in entries) {
                var entry = entries[entryKey];
                if (entries[entryKey].BIBTEXTYPEKEY == 'ARTICLE') {
    ...
    this.displayFeaturedWork = function (input, output, displayNum) {
        // parse bibtex input
        var b = new BibtexParser();
        b.setInput(input);
        try {
        ...
        var entries = b.getEntries();
        var entriesArray = this.createArrayPaper(entries);
        var sortedArray = this.sortArray(entriesArray, 'DATE', 'DESC', 'date');
        ...
        for (var entryKey in sortedArray) {
            ...
            if (entry['FEATURED'] == "1") {
                tpl = this.createTemplateWorks(entry, output, 'bibtex_template_fw', "col-lg-4 col-md-6 portfolio-item");
                // Check if template was created
                if (tpl) {
                    output.append(tpl);
                    tpl.show();
            ...
            if (displayCount > displayNum) break;
        }
        // remove old entries
        old.remove();
    }

    this.displayRecentWork = function (input, output, displayNum) {
        // parse bibtex input
        var b = new BibtexParser();
        b.setInput(input);
        try {
        ...
        var entriesArray = this.createArrayPaper(entries);
        var sortedArray = this.sortArray(entriesArray, 'DATE', 'DESC', 'date');
        var ssortedArray = sortedArray.slice(0, displayNum);
        // save old entries to remove them later
        var old = output.find("*");
        for (var entryKey in ssortedArray) {
            var entry = ssortedArray[entryKey];

            tpl = this.createTemplateWorks(entry, output, 'bibtex_template_rw', "col-lg-4 col-md-6 portfolio-item");
            // Check if template was created
            if (tpl) {
                output.append(tpl);
                tpl.show();
    ...
    this.displayPatents = function (input, output) {
        // parse bibtex input
        var b = new BibtexParser();
        b.setInput(input);
        try {
        ...
        var entriesArray = this.createArrayPatent(entries);
        var sortedArray = this.sortArray(entriesArray, 'DATE', 'DESC', 'date');
    ...
    this.displayAward = function (input, output) {
        // parse bibtex input
        var b = new BibtexParser();
        b.setInput(input);
        try {
        ...
        var entriesArray = this.createArrayAward(entries);
        var sortedArray = this.sortArray(entriesArray, 'DATE', 'DESC', 'date');
        // save old entries to remove them later
        var old = output.find("*");
        for (var entryKey in sortedArray) {
            ...
            tpl = this.createTemplateWorks(entry, output, 'bibtex_template_aw', 'bibtexentry');
            // Check if template was created
            if (tpl) {
                output.append(tpl);
                tpl.show();
...
function bibtex_js_draw() {
    ...
    $.when.apply($, requests).then(function () {
        ...
        //recent work
        if ($("#bibtex_recentwork").length) {
            var bibtex_renct = $("#bibtex_recentwork");
            (new BibtexDisplay()).displayRecentWork(bibstring, bibtex_renct, 6);

        }

        //patents
        if ($("#bibtex_patents").length) {
            var bibtex_renct = $("#bibtex_patents");
            (new BibtexDisplay()).displayPatents(bibstring, bibtex_renct);

        }

        //Award
        if ($("#bibtex_award").length) {
            var bibtex_renct = $("#bibtex_award");
